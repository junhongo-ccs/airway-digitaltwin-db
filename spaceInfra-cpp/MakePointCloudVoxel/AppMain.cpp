...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        logger->info("<<<<<<<<< MakePointCloudVoxel 処理開始(threadCount:{0}) >>>>>>>>", config.getThreadCount());
    ...
    logger->info("<<<<<<<<< MakePointCloudVoxel 処理終了 経過時間:{0}(秒) >>>>>>>>", (double)(clock() - t_start) / CLOCKS_PER_SEC);
